...
contract VerseReelNFT is ERC721Enumerable, Ownable, VRFConsumerBaseV2 {
    ...
    constructor(address _vrfCoordinatorV2Address, string memory name, string memory symbol) 
        ERC721(name, symbol) 
    ...
    function getTraits(uint256 _tokenId) public view returns (uint256[] memory) {
        return traits[_tokenId];
    }

    function reroll(uint8 _traitNumber, uint256 _tokenId) public {
        // check if user owns the nft before rerolling
        // currently rerolling is free, can either charge verse or use credit system
