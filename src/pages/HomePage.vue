...
export default {
  setup () {
    ...
    let truncatedAddress = computed(() => recognizableWalletFormat(accountRef.value.address))
    ...
    async function getAllowance () {
      try {
        ...
        const data = await readContract(core.wagmiConfig, {
          ...
          args: [accountRef.value.address, contractAddress]
        })
    ...
    const authChallenge = async () => {
      let resChallenge = await axios.get(
        `${GLOBALS.BACKEND_URL}/challenge/request/${accountRef.value.address}`
      )
      ...
      if (resChallenge.data.nonce) {
        ...
        let resComplete = await axios.post(
          `${GLOBALS.BACKEND_URL}/challenge/complete/${accountRef.value.address}`,
          payload
        )
        if (resComplete.data.token && resComplete.data.token.length > 1) {
          ...
          localStorage.setItem(
            `token/prod/${accountRef.value.address}`,
            JSON.stringify(item)
          )
    ...
    watchAccount(core.wagmiConfig, { async onChange(account) {
      ...
      if (account.address !== undefined) {
        const itemStr = localStorage.getItem(`token/prod/${account.address}`)
        if (!itemStr) {
          ...
          if (now.getTime() + 1200000 > item.expiry) {
            ...
            localStorage.removeItem(`token/prod/${account.address}`)
            authenticated.value = false
          } else {
            authenticated.value = true
            continueWithAccount()
...
<template>
  <div class="backdrop" v-if="buyModal">
    <p class="closeBuy">
      <i @click="closeBuy()" class="close-btn" style="cursor: pointer"></i>
    </p>
  ...
  <div class="backdrop" v-if="modalActive">
    ...
    <div class="modal" v-if="accountActive && !correctNetwork && !modalLoading">
      <div>
        <div class="modal-head">
        ...
        <div class="modal-body">
          ...
          <p class="subtext">
            Verse Voyager uses the Polygon network. Please change the network in
    ...
    <div class="modal" v-if="buyStep == 99 && !modalLoading">
      <div>
        ...
        <div class="modal-body">
          ...
          <a class="" target="_blank" @click="authChallenge()"
            ><button class="btn verse-wide">Sign Message</button></a
          >
          <p class="modal-footer">
            Signing in with your wallet proves your wallet ownership. The
            message you sign is a random short sentence
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="modal" v-if="buyStep == 3 && !modalLoading && correctNetwork">
      ...
      <div class="modal-body">
        ...
        <div class="gift-toggle-holder-bottom" v-if="giftTicket">
          <p>
            Please provide us with the Polygon wallet address of the person who
            you want to gift the character to.
          </p>
          <input
